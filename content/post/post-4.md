...
검색 엔진에 사이트가 검색에 노출되도록 할 것인지 제한할 것인지 설정하는 파일. 여기에 설정해놓은 검색엔진에 설정해놓은대로 노출된다.
...
검색 엔진이 탐색할때 사이트의 경로들을 참고할 수 있도록 제공하는 파일.
...
![](/img/post-4/2.png)
업로드해도 적용까지는 며칠 시간이 걸리는게 아닐까 싶다. 일단 기다려보는 수 밖에.
